...
func TestParseEntryRef(t *testing.T) {
	t.Run("name only", func(t *testing.T) {
		name, path := parseEntryRef("my-entry")
		assert.Equal(t, "my-entry", name)
		assert.Equal(t, "", path)
	})
...
func TestResolveVaultRef(t *testing.T) {
	...
	t.Run("name resolved", func(t *testing.T) {
		mockVault := newMockVaultClient(map[string]dvls.Vault{
			"my-vault": {Id: testVaultUUID, Name: "my-vault"},
		})
		id, err := resolveVaultRef(context.Background(), "my-vault", mockVault)
		assert.NoError(t, err)
		assert.Equal(t, testVaultUUID, id)
	})
...
func TestResolveEntryRef(t *testing.T) {
	...
	t.Run("name resolved", func(t *testing.T) {
		...
		entryID, err := c.resolveEntryRef(context.Background(), "my-entry")
		assert.NoError(t, err)
		assert.Equal(t, testEntryUUID, entryID)
	})
	...
	t.Run("path filters out other paths", func(t *testing.T) {
		...
		entryB := dvls.Entry{Id: "00000000-0000-0000-0000-000000000005", Name: "db", Path: `staging`, Type: dvls.EntryCredentialType, SubType: dvls.EntryCredentialSubTypeDefault}
		c, _ := newTestClient(map[string]dvls.Entry{testEntryUUID: entryA, "00000000-0000-0000-0000-000000000005": entryB})
	...
	t.Run("multiple entries found", func(t *testing.T) {
		entry2 := dvls.Entry{Id: "00000000-0000-0000-0000-000000000003", Name: "dup", Type: dvls.EntryCredentialType, SubType: dvls.EntryCredentialSubTypeDefault}
		entry3 := dvls.Entry{Id: "00000000-0000-0000-0000-000000000004", Name: "dup", Type: dvls.EntryCredentialType, SubType: dvls.EntryCredentialSubTypeConnectionString}
		c, _ := newTestClient(map[string]dvls.Entry{
			"00000000-0000-0000-0000-000000000003": entry2,
			"00000000-0000-0000-0000-000000000004": entry3,
		})
	...
	t.Run("GetEntries API error", func(t *testing.T) {
		...
		_, err := c.resolveEntryRef(context.Background(), "some-name")
		assert.Error(t, err)
...
func TestClient_GetSecret_ByName(t *testing.T) {
	entry := dvls.Entry{
		Id: testEntryUUID, Name: "my-secret",
		Type: dvls.EntryCredentialType, SubType: dvls.EntryCredentialSubTypeDefault,
		Data: &dvls.EntryCredentialDefaultData{Password: "name-resolved"},
	}
	c, _ := newTestClient(map[string]dvls.Entry{testEntryUUID: entry})

	val, err := c.GetSecret(context.Background(), esv1.ExternalSecretDataRemoteRef{Key: "my-secret", Property: "password"})
	assert.NoError(t, err)
...
func TestClient_GetSecretMap_ByName(t *testing.T) {
	...
	secretMap, err := c.GetSecretMap(context.Background(), esv1.ExternalSecretDataRemoteRef{Key: "my-secret"})
	assert.NoError(t, err)
...
func TestClient_SecretExists_ByName(t *testing.T) {
	entry := dvls.Entry{Id: testEntryUUID, Name: "my-entry", Type: dvls.EntryCredentialType, SubType: dvls.EntryCredentialSubTypeDefault}
	c, _ := newTestClient(map[string]dvls.Entry{testEntryUUID: entry})

	exists, err := c.SecretExists(context.Background(), pushSecretRemoteRefStub{remoteKey: "my-entry"})
	assert.NoError(t, err)
	assert.True(t, exists)
...
func TestClient_DeleteSecret_ByName(t *testing.T) {
	...
	err := c.DeleteSecret(context.Background(), pushSecretRemoteRefStub{remoteKey: "my-entry"})
	assert.NoError(t, err)
	assert.Equal(t, testEntryUUID, mockCred.lastDeleted)
}
...
func TestClient_PushSecret_ByName(t *testing.T) {
	...
	data := pushSecretDataStub{remoteKey: "my-entry", secretKey: "password"}
...
func TestIsNotFoundError(t *testing.T) {
	...
	assert.False(t, isNotFoundError(dvls.ErrVaultNotFound))
	assert.False(t, isNotFoundError(fmt.Errorf("wrapped: %w", dvls.ErrVaultNotFound)))
